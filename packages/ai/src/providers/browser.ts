...
export class BrowserProvider implements BrowserProviderInterface {
  ...
  constructor(config?: BrowserProviderConfig) {
    ...
    this.useWorker = mergedConfig.useWorker;
  ...
  async downloadModel(
    ...
    if (!model) {
      throw new AIError(
        ...
        `Model ${modelId} not found in curated list`
      );
    }
  ...
  /**
   * Check for cached models using the browser cache API.
   */
  private async checkCachedModels(): Promise<void> {
    ...
    try {
      ...
      for (const model of CURATED_MODELS) {
        const isCached = cacheNames.some(
          (name) =>
            name.includes(model.id) ||
            name.includes(model.id.toLowerCase().replace(/-/g, '_'))
        );

        if (isCached) {
          this.modelStatuses.set(model.id, { state: 'cached' });
        }
      }
    } catch {
      // Cache API may not be available, ignore
    }
  }
  ...
  private async loadWebLLM(): Promise<{
    ...
    try {
      ...
      throw new AIError(
        ...
        '@mlc-ai/web-llm is not installed. Install it with: pnpm add @mlc-ai/web-llm',
